...
{    
	...
	if (portal->item_type == ITEM_EXIT && portal->pIndexData->vnum == OBJ_VNUM_CHAIN) {
		act("$n climbs the chain.",ch,portal,NULL,TO_ROOM);
		act("You climb the chain.",ch,portal,NULL,TO_CHAR);

		char_from_room(ch);
		char_to_room(ch,get_room_index(ROOM_VNUM_CHAIN));
