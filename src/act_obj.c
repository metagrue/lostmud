...
void do_sacrifice( CHAR_DATA *ch, char *argument )
{
	    ...
	    if (!str_cmp("all",arg) || !str_prefix("all.",arg))
    {
      ...
      if (found) {
		...
		act( "$n sacrifices everything in the room to $G.", ch, obj, NULL, TO_ROOM );
...
CHAR_DATA *find_keeper( CHAR_DATA *ch )
{
    ...
    {
	//act("$N tells you 'We are still closed! We will open in a couple hours.'", ch, NULL, keeper, TO_CHAR);
	do_function(keeper, &do_say, "{aWe are still closed! We will open in a couple hours.{x" );
	return NULL;
    }
    else if ( time_info.hour == pShop->open_hour - 1 )
    {
	//act("$N tells you 'We aren't quite open yet! We will open in within the hour.'", ch, NULL, keeper, TO_CHAR);
	do_function(keeper, &do_say, "{aWe aren't quite open yet! We will open in within the hour.{x" );
	return NULL;
    }
    else if ( time_info.hour <  pShop->open_hour )
    {
	sprintf(buf, "{aSorry, we are closed. We will open at %d%s %s.{x",
	   pShop->open_hour, 
