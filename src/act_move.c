...
void move_char( CHAR_DATA *ch, int door, bool follow, bool quiet )
{
	...
	*/
	...
	if (!quiet)
	    act( "$n leaves $T.", ch, NULL, dir_name[door+6], TO_ROOM );
    ...
    {
	if (!quiet)
	    act( "$n arrives from $T.", ch, NULL, rev_move[door], TO_ROOM );
	if (IS_NPC(ch))
	{
	    if( ch->say_descr[0] != '\0')
	    {
		...
		else
		{
			...
			for (j=0; j < count; j++)
			{
				...
				if (j == count-1)
					...
					{
									printf_to_char(ch,"{C%s {W[%d] {D(%s){x\n\r",ch->in_room->name,ch->in_room->vnum, ch->in_room->area->name);
					} else {
									printf_to_char(ch,"{C%s {D(%s){x\n\r",ch->in_room->name, ch->in_room->area->name);
...
void do_recall( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( buf, ch );
	    ...
	    {
		send_to_char( "The {RWrath of Gaia {YZAPS{x your butt on the way out!\n\r", ch);
...
void do_train( CHAR_DATA *ch, char *argument )
{
    ...
    {	
	printf_to_char(ch, "Your %s is already at maximum. {x[%d]{x\n\r", 
                    pOutput, ch->perm_stat[stat]);
	return;
    }
    ...
    ch->perm_stat[stat]		+= 1;
		printf_to_char(ch, "Your %s increases. {x[%d/%d]{x\n\r", 
                    pOutput, ch->perm_stat[stat], get_max_train(ch,stat));
